...
bool GreyheartTidecallerMarkWaterElementalTotemAction::Execute(Event event)
{
    ...
    SerpentShrineCavernHelpers::MarkTargetWithSkull(bot, totem);
    return true;
}
...
bool HydrossTheUnstableNatureTankMoveBossToFrostTankAction::Execute(Event event)
{
    Unit* hydross = AI_VALUE2(Unit*, "find target", "hydross the unstable");
    if (!hydross)
        return false;
...
bool HydrossTheUnstableManageDPSTimerAction::Execute(Event event)
{
    ...
    if (!hydross->HasAura(SPELL_CORRUPTION) && 
        ...
        !bot->HasAura(SPELL_MARK_OF_CORRUPTION_250) || !bot->HasAura(SPELL_MARK_OF_CORRUPTION_500)))
    {
        if (hydrossDPSWaitTimer.find(mapId) == hydrossDPSWaitTimer.end())
            hydrossDPSWaitTimer[mapId] = time(nullptr);
    }

    if (bot->HasAura(SPELL_MARK_OF_HYDROSS_10) || bot->HasAura(SPELL_MARK_OF_CORRUPTION_10))
    {
        if (hydrossDPSWaitTimer.find(mapId) != hydrossDPSWaitTimer.end())
            hydrossDPSWaitTimer.erase(mapId);
    }
...
bool HydrossTheUnstableMarkElementalAddsAction::Execute(Event event)
{
    ...
    if (waterElemental)
        SerpentShrineCavernHelpers::MarkTargetWithSkull(bot, waterElemental);

    Unit* natureElemental = AI_VALUE2(Unit*, "find target", "tainted spawn of hydross");
    if (natureElemental)
        SerpentShrineCavernHelpers::MarkTargetWithSkull(bot, natureElemental);
