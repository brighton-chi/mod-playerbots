...
void RaidGruulsLairStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    triggers.push_back(new TriggerNode("gruul's lair high king maulgar", NextAction::array(0,
        ...
        new NextAction("high king maulgar control felstalker", ACTION_RAID + 5),
        new NextAction("high king maulgar subjugate felstalker", ACTION_RAID + 4),
        new NextAction("high king maulgar assign rti priority", ACTION_RAID + 3),
        new NextAction("high king maulgar blindeye tank", ACTION_RAID + 2),
        new NextAction("high king maulgar first off tank", ACTION_RAID + 2),
        new NextAction("high king maulgar main tank", ACTION_RAID + 2),
        new NextAction("high king maulgar hunter misdirection", ACTION_RAID + 1),
        nullptr)));
}
