...
void RaidKarazhanStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        ...
        new NextAction("karazhan netherspite block red beam", ACTION_RAID + 1),
        new NextAction("karazhan netherspite block blue beam", ACTION_RAID + 1),
        new NextAction("karazhan netherspite block green beam", ACTION_RAID + 1),
        nullptr)));

    triggers.push_back(new TriggerNode(
        "karazhan prince malchezaar", NextAction::array(0,
        new NextAction("karazhan prince malchezaar run away from shadow nova", ACTION_EMERGENCY + 1),
