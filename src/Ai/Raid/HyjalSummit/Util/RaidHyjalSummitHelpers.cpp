...
std::pair<size_t, size_t> GetBotCircleIndexAndCount(PlayerbotAI* botAI, Player* bot, const RangedGroups& groups)
{
...
Player* GetInfernoTarget(Unit* anetheron)
{
    ...
    if (spell && spell->m_spellInfo->Id == static_cast<uint32>(HyjalSummitSpells::SPELL_INFERNO))
    {
...
bool AnyGroupMemberHasDoom(Player* bot)
{
    if (Group* group = bot->GetGroup())
    {
        ...
        {
            Player* member = ref->GetSource();
            if (member && member->HasAura(static_cast<uint32>(HyjalSummitSpells::SPELL_DOOM)))
                return true;
        }
    }
