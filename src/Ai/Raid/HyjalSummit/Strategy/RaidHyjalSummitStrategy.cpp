...
void RaidHyjalSummitStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("rage winterchill bot is standing in death and decay",
                        {NextAction("rage winterchill move away from death and decay", ACTION_EMERGENCY + 2)}));

    // Anetheron
    triggers.push_back(
        new TriggerNode("anetheron pulling boss or infernal",
    ...
    triggers.push_back(
        new TriggerNode("anetheron bot is targeted by infernal",
                        {NextAction("anetheron bring infernal to infernal tank", ACTION_EMERGENCY + 2)}));

    triggers.push_back(
        new TriggerNode("anetheron infernals need to be kept away from raid",
    ...
    triggers.push_back(
        new TriggerNode("kaz'rogal bot is low on mana",
    ...
    // Azgalor
    triggers.push_back(
        new TriggerNode("azgalor pulling boss", {NextAction("azgalor misdirect boss to main tank", ACTION_RAID + 3)}));
    ...
    triggers.push_back(
        new TriggerNode("azgalor bot is doomed", {NextAction("azgalor move to doomguard tank", ACTION_EMERGENCY + 3)}));
